...
DROP TABLE IF EXISTS filter_1k.tag_edit_stats;

CREATE TABLE filter_1k.tag_edit_stats AS
  SELECT alledits.country_gid, alledits.key,
    num_users, 
    ...
    num_tag_values
  FROM (
    SELECT country_gid, key, count(distinct uid) as num_users,
    ...
    FROM (
      SELECT country_gid, uid, p.id as poi_id, key, value,
    ...
    GROUP BY country_gid, key
  ) alledits
  LEFT OUTER JOIN (
    SELECT country_gid, key, 
    ...
    GROUP BY country_gid, key
  ) ce ON (alledits.country_gid=ce.country_gid AND alledits.key=ce.key);
VACUUM ANALYZE filter_1k.tag_edit_stats;
