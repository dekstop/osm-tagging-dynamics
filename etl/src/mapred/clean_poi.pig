...
filtered_poi_id_group = COGROUP clean_node BY id INNER, clean_node_tag BY id INNER;
filtered_poi_id = FOREACH filtered_poi_id_group GENERATE $0;
store filtered_poi_id into '$output/poi_id';

-- poi
poi_join = COGROUP clean_node BY id INNER, filtered_poi_id BY $0 INNER;
poi = FOREACH poi_join GENERATE FLATTEN($1);
store poi into '$output/poi';

-- poi_tag
poi_tag_join = COGROUP clean_node_tag BY id INNER, filtered_poi_id BY $0 INNER;
